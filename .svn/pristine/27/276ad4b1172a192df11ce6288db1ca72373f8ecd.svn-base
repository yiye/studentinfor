    ...
    { 
        ...
        { 
            ...
            if (!empty($postStr)){ 
                ...
                <Content><![CDATA[回复 1 查看学生周知,回复<书名>进行图书检索,如果想聊天请关注isyiye]]></Content>";
                         }
...
查询结果在5分钟后失效";*/
        ...
        */
        public function nextPageBook($user,$q)
        {
          ...
          $userDB = new UserDB();
          $key = $userDB->getId($user);
          $c = new HandleMem();
          $five = $c->getBooksByTimes($key,$page);
          $result = "<MsgType><![CDATA[text]]></MsgType>
                     <Content><![CDATA[%s]]></Content>";
          $content = "";
          if ($five) {
